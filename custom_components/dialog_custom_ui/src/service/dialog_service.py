...
async def _resolve_llm_top_level_node(client_text, top_level_nodes, dialog_settings):
    dialog_cms_response = build_dialog_response(dialog_settings, "default_main")
    llm_system_prompt = str(dialog_cms_response.get("system", "")).strip()
    llm_context_prompt = _build_default_main_llm_context_prompt(client_text, top_level_nodes)
    llm_model = dialog_cms_response.get("model", "llama3")
    ...
    llm_response_norm = extract_json(llm_response)

    if not llm_response_norm:
        return None
    
    selected_id = llm_response_norm.get('id', None)
        
    selected_node = next((el for el in top_level_nodes if el["id"] == selected_id), None)

    return selected_node


def _build_default_search_prompt(client_text, search_settings):
    search_message = str(search_settings.get("message", "")).strip()
    if search_message and client_text:
        return f"{search_message}\nПользователь: {client_text}", str(search_settings.get("system", "")).strip()
    return client_text or search_message, str(search_settings.get("system", "")).strip()


async def _get_search_llm_response(client_text, dialog_settings):
    search_response = build_dialog_response(dialog_settings, "default_search")

    if not search_response:
        return None

    prompt, system = _build_default_search_prompt(client_text, search_response)
    if not prompt:
        return None
    ...
    try:
        raw_response = await asyncio.to_thread(
            ...
            prompt,
            system,
            search_response.get("model", "llama3"),
        )
    except Exception:
        return None
    ...
    end_status = search_response.get("endStatus", False)
