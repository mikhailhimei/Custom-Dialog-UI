var _="dialog_custom_ui/get_timer_alarm_config",g="dialog_custom_ui/save_timer_alarm_config";var b=[5,10,15,30,60],i=a=>String(a??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;"),h=a=>crypto?.randomUUID?`${a}${crypto.randomUUID().slice(0,8)}`:`${a}${Date.now()}`,n=a=>String(a).padStart(2,"0"),f=a=>{let t=new Date(String(a??"").replace(" ","T"));return Number.isNaN(t.getTime())?null:t},c=a=>`${n(Math.floor(a/3600))}:${n(Math.floor(a%3600/60))}:${n(a%60)}`,v=a=>{let t=String(a??"").split(":").map(e=>Number(e));return t.some(Number.isNaN)?0:t.length===3?t[0]*3600+t[1]*60+t[2]:t.length===2?t[0]*60+t[1]:t.length===1?t[0]*60:0},u=a=>{let t=a?.time&&typeof a.time=="object"?a.time:{},e=Number(t.total_seconds)||v(t.count_timer)||1800;return{id:String(a?.id??h("ha_timer:")),type:"timer",status:String(a?.status??"on"),clientId:String(a?.clientId??a?.userId??""),userId:String(a?.userId??a?.clientId??""),deviceId:String(a?.deviceId??a?.device_id??""),time:{count_timer:String(t.count_timer??c(e)),date_end:String(t.date_end??""),timezone:String(t.timezone??""),remaining_seconds:Number(t.remaining_seconds)||0,total_seconds:e}}},p=a=>{let t=a?.time&&typeof a.time=="object"?a.time:{};return{id:String(a?.id??h("ha_alarm:")),type:"alarm",status:String(a?.status??"on"),clientId:String(a?.clientId??a?.userId??""),userId:String(a?.userId??a?.clientId??""),deviceId:String(a?.deviceId??a?.device_id??""),time:{time:String(t.time??"08:00")}}},m=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._hass=null,this._tab="timer",this._items=[],this._presets=[],this._deviceLabels={},this._selectedDevice="",this._defaultMediaContentId="",this._loading=!1,this._saving=!1,this._error="",this._status="",this._tick=null,this._poll=null,this._debugWsPayload=null,this._theme="light"}set hass(t){this._hass=t,this._loading||this._load()}set config(t){let e=t&&typeof t=="object"?t:{};this._theme=String(e.theme??"light").toLowerCase()==="dark"?"dark":"light",this.setAttribute("data-theme",this._theme),this._render()}connectedCallback(){this.setAttribute("data-theme",this._theme),this.shadowRoot.innerHTML||this._render(),this._startLoops()}disconnectedCallback(){this._stopLoops()}async _load(){if(!(!this._hass||this._loading||this._saving)){this._loading=!0;try{let t=await this._hass.callWS({type:_});this._debugWsPayload=t;let e=Array.isArray(t?.items)?t.items:[],s=Array.isArray(t?.active_items)?t.active_items:[],r=new Map;[...e,...s].forEach(o=>{if(!o||typeof o!="object")return;let l=String(o.id??"");l&&r.set(l,{...r.get(l)||{},...o})}),this._items=[...r.values()].map(o=>String(o?.type??"").toLowerCase()==="timer"?u(o):p(o)),this._presets=Array.isArray(t?.alarm_presets)?t.alarm_presets.map(o=>String(o)):[],this._deviceLabels=t?.device_labels&&typeof t.device_labels=="object"?t.device_labels:{},this._defaultMediaContentId=String(t?.default_media_content_id??this._defaultMediaContentId??"").trim(),this._status=t?.last_updated?`Updated: ${t.last_updated}`:"",this._error=""}catch(t){this._debugWsPayload={ws_error:String(t?.message||t||"unknown error")},this._error=t?.message||"Failed to load timer/alarm state."}finally{this._loading=!1,this._render()}}}async _save(){if(!(!this._hass||this._saving)){this._saving=!0,this._error="",this._status="",this._render();try{await this._hass.callWS({type:g,timer_alarm_items:this._items,timer_alarm_media_content_id:this._defaultMediaContentId}),await this._load()}catch(t){this._error=t?.message||"Failed to save.",this._render()}finally{this._saving=!1,this._render()}}}_startLoops(){this._stopLoops(),this._tick=window.setInterval(()=>this._refreshLiveFields(),1e3),this._poll=window.setInterval(()=>{this._isInteractiveControlFocused()||this._load()},3e3)}_stopLoops(){this._tick&&(window.clearInterval(this._tick),this._tick=null),this._poll&&(window.clearInterval(this._poll),this._poll=null)}_devices(){return this._hass?.states?Object.values(this._hass.states).filter(t=>t.entity_id.startsWith("media_player.")).sort((t,e)=>(t.attributes.friendly_name||t.entity_id).localeCompare(e.attributes.friendly_name||e.entity_id)):[]}_timerLeft(t){let e=Math.max(0,Number(t?.time?.remaining_seconds)||0);if(t.status==="paused"||e>0)return e;let s=f(t.time.date_end);return s?Math.max(0,Math.floor((s.getTime()-Date.now())/1e3)):e}_isInteractiveControlFocused(){let t=this.shadowRoot?.activeElement;if(!t)return!1;let e=String(t.tagName||"").toUpperCase();return e==="SELECT"||e==="INPUT"||e==="TEXTAREA"}_nowTimeLabel(){let t=new Date;return`${n(t.getHours())}:${n(t.getMinutes())}:${n(t.getSeconds())}`}_refreshLiveFields(){this.shadowRoot?.querySelectorAll("[data-timer-left-id]").forEach(t=>{let e=String(t.getAttribute("data-timer-left-id")||""),s=this._items.find(r=>r.type==="timer"&&r.id===e);s&&(t.textContent=`${c(this._timerLeft(s))} left`)}),this.shadowRoot?.querySelectorAll("[data-now-clock]").forEach(t=>{t.textContent=this._nowTimeLabel()})}_renderDeviceOptions(t,e="Select media_player"){let s=this._devices(),r=t&&s.some(d=>d.entity_id===t),o=String(this._deviceLabels?.[t]||"").trim(),l=r||!t?"":`<option value="${i(t)}" selected>${i(o||t)} (${i(t)})</option>`;return`
      <option value="">${i(e)}</option>
      ${l}
      ${s.map(d=>`<option value="${i(d.entity_id)}" ${t===d.entity_id?"selected":""}>${i(d.attributes.friendly_name||d.entity_id)}</option>`).join("")}
    `}_addTimer(t){let e=Math.max(60,Number(t)*60||300),s=new Date(Date.now()+e*1e3),r=u({id:h("ha_timer:"),status:"on",deviceId:this._selectedDevice||this._devices()[0]?.entity_id||"",time:{count_timer:c(e),date_end:`${s.getFullYear()}-${n(s.getMonth()+1)}-${n(s.getDate())} ${n(s.getHours())}:${n(s.getMinutes())}:${n(s.getSeconds())}`,remaining_seconds:e,total_seconds:e}});this._items=[...this._items,r],this._save()}_addAlarm(t="08:00"){let e=p({id:h("ha_alarm:"),status:"on",deviceId:this._selectedDevice||this._devices()[0]?.entity_id||"",time:{time:t}});this._items=[...this._items,e],this._save()}_updateItem(t,e){this._items=this._items.map(s=>s.id===t?{...s,...e}:s),this._save()}_removeItem(t){this._items=this._items.filter(e=>e.id!==t),this._save()}_pauseTimer(t){this._items=this._items.map(e=>e.id!==t||e.type!=="timer"?e:{...e,status:"paused",time:{...e.time,remaining_seconds:this._timerLeft(e)}}),this._save()}_resumeTimer(t){this._items=this._items.map(e=>{if(e.id!==t||e.type!=="timer")return e;let s=Math.max(1,Number(e.time.remaining_seconds)||60),r=new Date(Date.now()+s*1e3);return{...e,status:"on",time:{...e.time,date_end:`${r.getFullYear()}-${n(r.getMonth()+1)}-${n(r.getDate())} ${n(r.getHours())}:${n(r.getMinutes())}:${n(r.getSeconds())}`,count_timer:c(Math.max(s,Number(e.time.total_seconds)||s))}}}),this._save()}_renderTimers(){let t=this._items.filter(e=>e.type==="timer");return t.length?t.map(e=>{let s=this._timerLeft(e);return`
        <article class="card">
          <div class="head">
            <div>
              <div class="title">Timer</div>
              <div class="meta" data-timer-left-id="${i(e.id)}">${i(c(s))} left</div>
              <div class="meta">Current time: <span data-now-clock>${i(this._nowTimeLabel())}</span></div>
            </div>
            <div class="actions">
    ...
    `}_bind(){this.shadowRoot.querySelectorAll("[data-tab]").forEach(t=>{t.onclick=()=>{this._tab=t.dataset.tab,this._render()}}),this.shadowRoot.querySelectorAll('[data-action="quick-timer"]').forEach(t=>{t.onclick=()=>this._addTimer(Number(t.dataset.minutes)||5)}),this.shadowRoot.querySelector('[data-action="add-alarm"]')?.addEventListener("click",()=>this._addAlarm("08:00")),this.shadowRoot.querySelectorAll('[data-action="add-alarm-preset"]').forEach(t=>{t.onclick=()=>this._addAlarm(String(t.dataset.preset||"08:00"))}),this.shadowRoot.querySelector('[data-action="select-global-device"]')?.addEventListener("change",t=>{this._selectedDevice=String(t.currentTarget.value||"")}),this.shadowRoot.querySelector('[data-action="set-global-media"]')?.addEventListener("change",t=>{this._defaultMediaContentId=String(t.currentTarget.value||"").trim(),this._save()}),this.shadowRoot.querySelectorAll('[data-action="remove-item"]').forEach(t=>{t.onclick=()=>this._removeItem(String(t.dataset.itemId||""))}),this.shadowRoot.querySelectorAll('[data-action="pause-timer"]').forEach(t=>{t.onclick=()=>this._pauseTimer(String(t.dataset.itemId||""))}),this.shadowRoot.querySelectorAll('[data-action="resume-timer"]').forEach(t=>{t.onclick=()=>this._resumeTimer(String(t.dataset.itemId||""))}),this.shadowRoot.querySelectorAll('[data-action="set-device"]').forEach(t=>{t.onchange=e=>this._updateItem(String(t.dataset.itemId||""),{deviceId:String(e.currentTarget.value||"")})}),this.shadowRoot.querySelectorAll('[data-action="set-alarm-time"]').forEach(t=>{t.onchange=e=>{let s=String(e.currentTarget.value||"08:00"),r=String(t.dataset.itemId||"");this._items=this._items.map(o=>o.id===r?{...o,time:{...o.time,time:s}}:o),this._save()}}),this.shadowRoot.querySelectorAll('[data-action="set-alarm-enabled"]').forEach(t=>{t.onchange=e=>this._updateItem(String(t.dataset.itemId||""),{status:e.currentTarget.checked?"on":"off"})})}_render(){let t=this._tab==="timer"?this._renderTimers():this._renderAlarms(),e=`
      ...
      <div class="panel">
        <section class="hero">
          ...
          <div class="meta" style="margin-top:8px;">Current time: <span data-now-clock>${i(this._nowTimeLabel())}</span></div>
          <div class="toolbar" style="margin-top:10px;">
            <select data-action="select-global-device">
              ${this._renderDeviceOptions(this._selectedDevice,"Device for quick start")}
            </select>
            ${b.map(s=>`<button type="button" class="btn ghost" data-action="quick-timer" data-minutes="${s}">+${s}m</button>`).join("")}
          ...
          <label class="media-field">
            ...
            <input data-action="set-global-media" value="${i(this._defaultMediaContentId)}" placeholder="media-source://media_source/local/your-song.mp3" />
          </label>
          ...
          <details style="margin-top:10px;">
            ...
            <pre style="margin-top:8px; max-height:220px; overflow:auto; background:var(--ta-bg-soft); border:1px solid var(--ta-border); border-radius:8px; padding:8px;">${i(JSON.stringify(this._debugWsPayload??{},null,2))}</pre>
          </details>
        </section>
        ${t}
      </div>
    `;this.shadowRoot.innerHTML=e,this._bind(),this._refreshLiveFields()}};customElements.get("dialog-custom-ui-timer-alarm")||customElements.define("dialog-custom-ui-timer-alarm",m);
