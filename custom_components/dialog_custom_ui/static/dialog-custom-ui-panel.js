...
`;var Gp=e=>{let t=e._renderActiveTabBody(),n=`
  ...
  `;var C=e=>String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;"),ig=()=>globalThis.crypto?.randomUUID?globalThis.crypto.randomUUID():`scenario_${Date.now()}_${Math.random().toString(16).slice(2,10)}`,Ll=()=>globalThis.crypto?.randomUUID?globalThis.crypto.randomUUID():`condition_${Date.now()}_${Math.random().toString(16).slice(2,10)}`,yn=()=>({id:Ll(),parent_type:"",children_type:"",children_type_enabled:!1,children_direct_type:"",children_direct_type_enabled:!1}),sf=()=>({id:ig(),name:"",script_entity_id:"",conditions:[yn()]});var Qi=e=>{let t=e._buildConfigPayload();return`
      <section class="hero-card">
        <h1>JSON</h1>
    ...
    <div class="yandex-tabs-toolbar">
      <label class="yandex-scenario-select">
        ...
        <select data-action="select-yandex-tab-dropdown">
          ${t.map((i,a)=>{let o=String(i.mainCommand??"").trim(),l=i.mainCommand||`\u0421\u0446\u0435\u043D\u0430\u0440\u0438\u0439 ${a+1}`;return`<option value="${C(o)}" ${n===o?"selected":""}>${C(l)}</option>`}).join("")}
