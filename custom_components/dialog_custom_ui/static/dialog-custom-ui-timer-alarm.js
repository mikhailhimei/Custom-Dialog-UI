var p="dialog_custom_ui/get_timer_alarm_config",_="dialog_custom_ui/save_timer_alarm_config";var g=[5,10,15,30,60],i=s=>String(s??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;"),c=s=>crypto?.randomUUID?`${s}${crypto.randomUUID().slice(0,8)}`:`${s}${Date.now()}`,n=s=>String(s).padStart(2,"0"),b=s=>{let t=new Date(String(s??"").replace(" ","T"));return Number.isNaN(t.getTime())?null:t},l=s=>`${n(Math.floor(s/3600))}:${n(Math.floor(s%3600/60))}:${n(s%60)}`,f=s=>{let t=String(s??"").split(":").map(e=>Number(e));return t.some(Number.isNaN)?0:t.length===3?t[0]*3600+t[1]*60+t[2]:t.length===2?t[0]*60+t[1]:t.length===1?t[0]*60:0},u=s=>{let t=s?.time&&typeof s.time=="object"?s.time:{},e=Number(t.total_seconds)||f(t.count_timer)||1800;return{id:String(s?.id??c("ha_timer:")),type:"timer",status:String(s?.status??"on"),clientId:String(s?.clientId??s?.userId??""),userId:String(s?.userId??s?.clientId??""),deviceId:String(s?.deviceId??s?.device_id??""),time:{count_timer:String(t.count_timer??l(e)),date_end:String(t.date_end??""),timezone:String(t.timezone??""),remaining_seconds:Number(t.remaining_seconds)||0,total_seconds:e}}},m=s=>{let t=s?.time&&typeof s.time=="object"?s.time:{};return{id:String(s?.id??c("ha_alarm:")),type:"alarm",status:String(s?.status??"on"),clientId:String(s?.clientId??s?.userId??""),userId:String(s?.userId??s?.clientId??""),deviceId:String(s?.deviceId??s?.device_id??""),time:{time:String(t.time??"08:00")}}},h=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._hass=null,this._tab="timer",this._items=[],this._presets=[],this._selectedDevice="",this._loading=!1,this._saving=!1,this._error="",this._status="",this._tick=null,this._poll=null,this._debugWsPayload=null}set hass(t){this._hass=t,this._loading||this._load()}connectedCallback(){this.shadowRoot.innerHTML||this._render(),this._startLoops()}disconnectedCallback(){this._stopLoops()}async _load(){if(!(!this._hass||this._loading||this._saving)){this._loading=!0;try{let t=await this._hass.callWS({type:p});this._debugWsPayload=t;let e=Array.isArray(t?.items)?t.items:[],a=Array.isArray(t?.active_items)?t.active_items:[],r=new Map;[...e,...a].forEach(o=>{if(!o||typeof o!="object")return;let d=String(o.id??"");d&&r.set(d,{...r.get(d)||{},...o})}),this._items=[...r.values()].map(o=>String(o?.type??"").toLowerCase()==="timer"?u(o):m(o)),this._presets=Array.isArray(t?.alarm_presets)?t.alarm_presets.map(o=>String(o)):[],this._status=t?.last_updated?`Updated: ${t.last_updated}`:"",this._error=""}catch(t){this._debugWsPayload={ws_error:String(t?.message||t||"unknown error")},this._error=t?.message||"Failed to load timer/alarm state."}finally{this._loading=!1,this._render()}}}async _save(){if(!(!this._hass||this._saving)){this._saving=!0,this._error="",this._status="",this._render();try{await this._hass.callWS({type:_,timer_alarm_items:this._items}),await this._load()}catch(t){this._error=t?.message||"Failed to save.",this._render()}finally{this._saving=!1,this._render()}}}_startLoops(){this._stopLoops(),this._tick=window.setInterval(()=>this._refreshLiveFields(),1e3),this._poll=window.setInterval(()=>{this._isInteractiveControlFocused()||this._load()},3e3)}_stopLoops(){this._tick&&(window.clearInterval(this._tick),this._tick=null),this._poll&&(window.clearInterval(this._poll),this._poll=null)}_devices(){return this._hass?.states?Object.values(this._hass.states).filter(t=>t.entity_id.startsWith("media_player.")).sort((t,e)=>(t.attributes.friendly_name||t.entity_id).localeCompare(e.attributes.friendly_name||e.entity_id)):[]}_timerLeft(t){let e=Math.max(0,Number(t?.time?.remaining_seconds)||0);if(t.status==="paused"||e>0)return e;let a=b(t.time.date_end);return a?Math.max(0,Math.floor((a.getTime()-Date.now())/1e3)):e}_isInteractiveControlFocused(){let t=this.shadowRoot?.activeElement;if(!t)return!1;let e=String(t.tagName||"").toUpperCase();return e==="SELECT"||e==="INPUT"||e==="TEXTAREA"}_nowTimeLabel(){let t=new Date;return`${n(t.getHours())}:${n(t.getMinutes())}:${n(t.getSeconds())}`}_refreshLiveFields(){this.shadowRoot?.querySelectorAll("[data-timer-left-id]").forEach(t=>{let e=String(t.getAttribute("data-timer-left-id")||""),a=this._items.find(r=>r.type==="timer"&&r.id===e);a&&(t.textContent=`${l(this._timerLeft(a))} left`)}),this.shadowRoot?.querySelectorAll("[data-now-clock]").forEach(t=>{t.textContent=this._nowTimeLabel()})}_renderDeviceOptions(t,e="Select media_player"){let a=this._devices(),o=t&&a.some(d=>d.entity_id===t)||!t?"":`<option value="${i(t)}" selected>device_id: ${i(t)}</option>`;return`
      <option value="">${i(e)}</option>
      ${o}
      ${a.map(d=>`<option value="${i(d.entity_id)}" ${t===d.entity_id?"selected":""}>${i(d.attributes.friendly_name||d.entity_id)}</option>`).join("")}
    `}_addTimer(t){let e=Math.max(60,Number(t)*60||300),a=new Date(Date.now()+e*1e3),r=u({id:c("ha_timer:"),status:"on",deviceId:this._selectedDevice||this._devices()[0]?.entity_id||"",time:{count_timer:l(e),date_end:`${a.getFullYear()}-${n(a.getMonth()+1)}-${n(a.getDate())} ${n(a.getHours())}:${n(a.getMinutes())}:${n(a.getSeconds())}`,remaining_seconds:e,total_seconds:e}});this._items=[...this._items,r],this._save()}_addAlarm(t="08:00"){let e=m({id:c("ha_alarm:"),status:"on",deviceId:this._selectedDevice||this._devices()[0]?.entity_id||"",time:{time:t}});this._items=[...this._items,e],this._save()}_updateItem(t,e){this._items=this._items.map(a=>a.id===t?{...a,...e}:a),this._save()}_removeItem(t){this._items=this._items.filter(e=>e.id!==t),this._save()}_pauseTimer(t){this._items=this._items.map(e=>e.id!==t||e.type!=="timer"?e:{...e,status:"paused",time:{...e.time,remaining_seconds:this._timerLeft(e)}}),this._save()}_resumeTimer(t){this._items=this._items.map(e=>{if(e.id!==t||e.type!=="timer")return e;let a=Math.max(1,Number(e.time.remaining_seconds)||60),r=new Date(Date.now()+a*1e3);return{...e,status:"on",time:{...e.time,date_end:`${r.getFullYear()}-${n(r.getMonth()+1)}-${n(r.getDate())} ${n(r.getHours())}:${n(r.getMinutes())}:${n(r.getSeconds())}`,count_timer:l(Math.max(a,Number(e.time.total_seconds)||a))}}}),this._save()}_renderTimers(){let t=this._items.filter(e=>e.type==="timer");return t.length?t.map(e=>{let a=this._timerLeft(e);return`
        <article class="card">
          <div class="head">
            <div>
              <div class="title">Timer</div>
              <div class="meta" data-timer-left-id="${i(e.id)}">${i(l(a))} left</div>
      ...
      <div class="panel">
        <section class="hero">
          ...
          <div class="toolbar" style="margin-top:10px;">
            ...
            ${g.map(a=>`<button type="button" class="btn ghost" data-action="quick-timer" data-minutes="${a}">+${a}m</button>`).join("")}
    ...
    `;this.shadowRoot.innerHTML=e,this._bind(),this._refreshLiveFields()}};customElements.get("dialog-custom-ui-timer-alarm")||customElements.define("dialog-custom-ui-timer-alarm",h);
